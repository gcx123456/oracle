...
SQL>ALTER USER gcx_user QUOTA UNLIMITED ON USERS;
SQL>ALTER USER gcx_user QUOTA UNLIMITED ON USERS02;
SQL>ALTER USER gcx_user QUOTA UNLIMITED ON USERS03;
SQL>exit
```
![](t1.png)
#### 然后用gcx_user来实现创建表
...
#### MAX_MB是指数据文件的最大容量。
## 实验总结：
#### 这次的实验的用户名就是使用上次实验二创建的用户名，由于上次实验的时候，我自己使用的数据库名字不是这个，导致这次出现失误，又返回到实验二上面重新建立了一个用户名，重点就是在实验二的时候，在建立用户的时候需要给用户权限，不然在实验三的时候会有问题，这次的实验学会了建立表的正确使用方法，也知道了分区策略的设计，以及知道了在建立表的时候的SQL语句的使用情况及后面对数据库使用情况的查看。